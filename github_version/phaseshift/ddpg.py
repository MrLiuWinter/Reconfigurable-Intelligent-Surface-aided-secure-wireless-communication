...
class Actor(nn.Module):
    def __init__(self, n_states, n_actions, hidden_dim1, hidden_dim2):
        super(Actor, self).__init__()
        self.linear1 = nn.Linear(n_states, hidden_dim1)
        self.linear2 = nn.Linear(hidden_dim1, hidden_dim2)
        self.linear3 = nn.Linear(hidden_dim2, n_actions)
...
class Critic(nn.Module):
    def __init__(self, n_states, n_actions, hidden_dim1, hidden_dim2):
        ...
        self.linear1 = nn.Linear(n_states + n_actions, hidden_dim1)
        self.linear2 = nn.Linear(hidden_dim1, hidden_dim2)
...
class DDPG:
    def __init__(self, n_states, n_actions, cfg):
        ...
        self.critic = Critic(n_states, n_actions, cfg.hidden_dim_1, cfg.hidden_dim_2).to(cfg.device)
        # critic——输入state和actor的输出(action)，得到一个scalar
        self.actor = Actor(n_states, n_actions, cfg.hidden_dim_1, cfg.hidden_dim_2).to(cfg.device)
        # actor——输入state,输出离散的action
        self.target_critic = Critic(n_states, n_actions, cfg.hidden_dim_1, cfg.hidden_dim_2).to(cfg.device)
        self.target_actor = Actor(n_states, n_actions, cfg.hidden_dim_1, cfg.hidden_dim_2).to(cfg.device)
        ...
        self.critic_optimizer = optim.Adam(self.critic.parameters(), lr=cfg.critic_lr, weight_decay=1e-5)
        self.actor_optimizer = optim.Adam(self.actor.parameters(), lr=cfg.actor_lr, weight_decay=1e-5)
        # actor和critic的优化器
